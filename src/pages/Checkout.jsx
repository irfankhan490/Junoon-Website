...
import { Plus, Minus, Trash2, ShoppingBag, Truck, Landmark, Smartphone, CheckCircle2, ArrowLeft, ArrowRight, AlertTriangle, MessageCircle } from 'lucide-react'
...
export default function Checkout() {
  ...
  const previousAttemptRef = useRef({})
  ...
  const handleConfirm = async () => {
    // Guard with a ref (checked synchronously) rather than relying solely on the
    // `submitting` state, since state updates from setSubmitting(true) aren't
    // guaranteed to be applied before a near-simultaneous second click/re-render.
    ...
    try {
      const order = await placeOrder(
        { customer: form, payment, items, subtotal, deliveryFee, total, totalWeightKg },
        previousAttemptRef.current
      )
      clearCart()
      navigate('/order-confirmation', { state: { order } })
    } catch (err) {
      // Remember the order number/timestamp from this attempt (if any) so a
      // retry reuses it instead of generating a second, conflicting order number.
      previousAttemptRef.current = err?.orderNumber
        ? { orderNumber: err.orderNumber, placedAt: err.placedAt }
        : previousAttemptRef.current
      setSubmitError(
        err?.message || "We couldn't complete your order right now. Please try again or contact us on WhatsApp."
      )
    } finally {
  ...
  const whatsappFallbackText = [
    `Assalam-o-Alaikum, I tried to place an order on the website but it couldn't be submitted online. Here are my details:`,
        ...
        <div className="mt-10">
          ...
          {step === 1 && (
            <form className="card grid gap-5 p-6 sm:p-8" onSubmit={(e) => { e.preventDefault(); goNext() }}>
              ...
              <div>
                <label className="label-field" htmlFor="email">Email (optional)</label>
                <input id="email" type="email" className="input-field" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="you@example.com" />
                {errors.email && <p className="mt-1 text-xs font-semibold text-chili">{errors.email}</p>}
                <p className="mt-1 text-xs text-espresso-soft/50">We’ll send your order confirmation here if you provide it.</p>
              </div>
              <div>
          ...
          {step === 2 && (
            <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
              ...
              <div className="card h-fit p-6 sm:p-8">
                ...
                {submitError && (
                  <div className="mt-5 rounded-2xl border-2 border-chili/30 bg-chili/5 p-4">
                    <p className="flex items-start gap-2 text-sm font-semibold text-chili">
                      <AlertTriangle size={16} className="mt-0.5 shrink-0" /> {submitError}
                    </p>
                    <a
                      href={waLink(BUSINESS.whatsapp[0], whatsappFallbackText)}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="btn-dark mt-3 w-full justify-center"
                    >
                      <MessageCircle size={16} /> Send Order via WhatsApp Instead
                    </a>
                  </div>
                )}
